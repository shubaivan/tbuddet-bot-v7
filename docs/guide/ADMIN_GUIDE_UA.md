...
8. [Реєстрація та підтвердження email](#8-реєстрація-та-підтвердження-email)
9. [Сповіщення менеджерів](#9-сповіщення-менеджерів)
...
Замовлення — основний розділ для менеджера. Тут ви бачите всі покупки клієнтів (з сайту та Telegram-бота), управляєте їх статусами, додаєте ТТН для доставки та переглядаєте деталі оплати.

### 5.1. Звідки приходять замовлення?

Замовлення створюються двома шляхами:

| Джерело | Як це працює |
|---------|-------------|
| **Telegram-бот** | Клієнт натискає /start → обирає товар → властивості → місто → відділення → оплачує через LiqPay. Замовлення з'являється в адмін-панелі автоматично. |
| **Сайт (artbeton.market)** | Клієнт реєструється → додає товар у кошик → оформлює → оплачує через LiqPay. Замовлення з'являється в адмін-панелі автоматично. |

### 5.2. Як менеджер дізнається про нове замовлення?

Коли клієнт **оплачує** замовлення, система автоматично надсилає **Telegram-повідомлення** всім менеджерам:

```
Нове замовлення #35
Сума: 4850 грн
Місто: Черкаси
Відділення: №12 (до 30 кг на одне місце): вул. Молоткова, 20
Клієнт: Лена Гураль, +380937542639

Відкрити замовлення  ← клікабельне посилання
```

**Натисніть "Відкрити замовлення"** — відкриється сторінка замовлення в адмін-панелі, де можна побачити всі деталі та почати обробку.

> Повідомлення отримують всі користувачі з роллю МЕНЕДЖЕР, які зареєстровані в Telegram-боті.

### 5.3. Список замовлень

**URL:** [/admin/orders](https://api.artbeton.market/admin/orders)

**Фільтри** (верхня панель):

| Фільтр | Для чого |
|--------|----------|
| **Статус замовлення** | Показати тільки нові / в обробці / відправлені / доставлені / скасовані |
| **Оплата** | Показати тільки оплачені / не оплачені / очікують |
| **Дата від — до** | Знайти замовлення за конкретний період |
| **Пошук** | Шукає по всіх полях: ім'я клієнта, телефон, місто, товар, ТТН |

Натисніть **"Фільтрувати"** для застосування або **"Скинути"** для очищення.

**Колонки таблиці:**

| Колонка | Що показує |
|---------|-----------|
| **id** | Номер замовлення — **натисніть щоб відкрити деталі** |
| **total_amount** | Загальна сума (грн) |
| **order_status** | Поточний статус (кольоровий індикатор) |
| **nova_poshta_tracking_number** | ТТН — **натисніть щоб відстежити на сайті Нової Пошти** |
| **delivery_city** | Місто доставки |
| **delivery_department** | Відділення Нової Пошти |
| **liq_pay_status** | Статус оплати: success = оплачено |
| **product_info** | Назва товару та кількість |
| **c_user_info** | Клієнт з сайту (ім'я, телефон) |
| **t_user_info** | Клієнт з Telegram (ім'я, username) |
| **created_at** | Дата створення |

> **Порада:** Щоб швидко знайти конкретне замовлення — введіть номер телефону клієнта або номер замовлення в поле "Пошук".

### 5.4. Сторінка деталей замовлення

**URL:** /admin/orders/{id} (наприклад /admin/orders/35)

Натисніть на номер замовлення в списку або перейдіть за посиланням з Telegram-повідомлення.

Сторінка містить **5 блоків**:

#### Блок 1: Інформація
| Поле | Що показує |
|------|-----------|
| **Сума** | Загальна вартість замовлення в грн |
| **Статус оплати** | Оплачено (зелений) / Очікує (сірий) / інший статус |
| **Дата створення** | Коли клієнт оформив замовлення |
| **Останнє оновлення** | Коли менеджер останній раз змінював статус |

#### Блок 2: Клієнт
Показує інформацію про покупця:
- **Ім'я, телефон, email** — якщо замовлення з сайту
- **Telegram ім'я, username, телефон** — якщо замовлення з бота
- Може бути обидва, якщо акаунти пов'язані

> Використовуйте телефон клієнта щоб зв'язатися для уточнення деталей замовлення.

#### Блок 3: Доставка (Нова Пошта)
- **Місто** — куди відправляти
- **Відділення** — конкретне відділення Нової Пошти

#### Блок 4: Товари
- Список усіх товарів у замовленні
- Кожен товар показує: **назву**, **кількість**, **обрані властивості** (колір, розмір, утеплювач тощо)
- Якщо властивість має доплату — вона показана в дужках
- **Опис** — повний текст замовлення з деталями

#### Блок 5: LiqPay (оплата)
- **LiqPay Order ID** — унікальний ідентифікатор платежу
- **Статус оплати** — Оплачено / Очікує / інший
- **Відповідь LiqPay** — повна технічна інформація від платіжної системи (JSON формат)

> Якщо клієнт каже що оплатив але статус не "success" — перевірте відповідь LiqPay у цьому блоці. Там буде причина відхилення.

#### Блок 6: Управління замовленням
Тут менеджер працює із замовленням:

| Поле | Для чого |
|------|----------|
| **Статус замовлення** | Змініть на потрібний етап (див. нижче) |
| **Номер ТТН (Нова Пошта)** | Введіть номер накладної після відправки |
| **Кнопка "Зберегти"** | Збережіть зміни |

Якщо ТТН вже введено — під полем з'явиться посилання **"Відстежити посилку →"** на сайт Нової Пошти.

### 5.5. Як обробляти замовлення (покрокова інструкція)

**Крок 1: Отримали повідомлення в Telegram**
- Натисніть "Відкрити замовлення" в повідомленні
- Або зайдіть в [адмін-панель → Замовлення](https://api.artbeton.market/admin/orders) та знайдіть нове

**Крок 2: Перевірте оплату**
- На сторінці замовлення перевірте блок "LiqPay"
- Статус повинен бути **"Оплачено"** (зелений)
- Якщо не оплачено — зачекайте або зв'яжіться з клієнтом

**Крок 3: Зв'яжіться з клієнтом**
- Подзвоніть за номером телефону з блоку "Клієнт"
- Уточніть деталі: колір, розмір, додаткові побажання
- Узгодьте терміни виготовлення та доставки

**Крок 4: Змініть статус на "В обробці"**
- У блоці "Управління замовленням" виберіть **"processing"** (В обробці)
- Натисніть **"Зберегти"**
- Клієнт отримає повідомлення в Telegram про зміну статусу

**Крок 5: Підготуйте та відправте**
- Виготовіть/підготуйте товар
- Відправте через Нову Пошту
- Запишіть номер ТТН (накладної)

**Крок 6: Введіть ТТН та змініть статус**
- На сторінці замовлення введіть **номер ТТН** у відповідне поле
- Змініть статус на **"shipped"** (Відправлено)
- Натисніть **"Зберегти"**
- Клієнт отримає повідомлення з ТТН і зможе відстежити посилку

**Крок 7: Замовлення доставлено**
- Коли клієнт отримав посилку — змініть статус на **"delivered"** (Доставлено)
- Натисніть **"Зберегти"**

**Скасування замовлення:**
- Якщо потрібно скасувати — виберіть статус **"cancelled"** (Скасовано)
- Натисніть **"Зберегти"**

### 5.6. Статуси замовлення

```
Нове (new)
  ↓  Перевірити оплату, зв'язатися з клієнтом
В обробці (processing)
  ↓  Виготовити та відправити
Відправлено (shipped) + ТТН
  ↓  Клієнт отримав
Доставлено (delivered)
```

| Статус | Значення | Коли ставити |
|--------|----------|-------------|
| **new** | Нове замовлення | Автоматично при створенні |
| **processing** | В обробці | Після перевірки оплати та зв'язку з клієнтом |
| **shipped** | Відправлено | Після відправки Новою Поштою (обов'язково вказати ТТН!) |
| **delivered** | Доставлено | Коли клієнт підтвердив отримання |
| **cancelled** | Скасовано | Якщо замовлення скасовано |

> **Важливо:** При кожній зміні статусу клієнт отримує повідомлення в Telegram. Переконайтесь що статус правильний перед збереженням.

### 5.7. Швидка навігація по замовленнях

| Що потрібно | Де знайти |
|-------------|-----------|
| Нові необроблені замовлення | Фільтр "Статус" → "Нове" + "Оплата" → "Оплачено" |
| Замовлення конкретного клієнта | Поле "Пошук" → ввести телефон або ім'я |
| Замовлення за сьогодні | Фільтр "Дата від" → сьогоднішня дата |
| Відправлені без ТТН | Фільтр "Статус" → "Відправлено", перевірити колонку ТТН |
| Перевірити доставку | Натиснути на ТТН у таблиці → відкриється трекінг Нової Пошти |
...
## 8. Реєстрація та підтвердження email

### 8.1. Як зареєструватися на сайті

1. Відкрийте [artbeton.market](https://artbeton.market)
2. Натисніть **"Увійти"** (вгорі справа)
3. Натисніть **"Зареєструватись"**
4. Заповніть форму:
   - **Email** — ваша електронна пошта
   - **Ім'я** — ваше ім'я
   - **Телефон** — оберіть країну з прапорцем та введіть номер (Україна за замовчуванням)
5. Натисніть **"Зареєструватись"**
6. Побачите екран **"Перевірте вашу пошту"**

### 8.2. Підтвердження email

1. Перевірте свою пошту (лист від "Buddet")
2. Натисніть посилання **"Підтвердити email"** у листі
3. Відкриється сторінка з полями для пароля
4. Введіть пароль (мінімум 8 символів) та підтвердіть його
5. Натисніть **"Зберегти пароль"**
6. Тепер можна увійти на сайт з вашим email та паролем

> **Важливо:** Посилання дійсне **24 години**. Якщо термін закінчився — зареєструйтесь заново.

---

## 9. Сповіщення менеджерів

### 9.1. Telegram-сповіщення про оплату

Коли клієнт оплачує замовлення (через бот або сайт), всі менеджери отримують Telegram-повідомлення:

```
Нове замовлення #123
Сума: 15200 грн
Місто: Черкаси
Відділення: №12 (до 30 кг)
Клієнт: Іван Левін, +380633022666

Відкрити замовлення (посилання на адмін-панель)
```

Натисніть **"Відкрити замовлення"** щоб перейти на сторінку замовлення в адмін-панелі.

### 9.2. Хто отримує сповіщення

Сповіщення отримують всі користувачі з роллю **ROLE_MANAGER** які мають Telegram chat_id.
